...
from utils.db_utils import get_quiz_questions, record_user_score, get_quiz_name, has_taken_quiz

logger = logging.getLogger('badgey.solo_quiz_dm')

class DMQuizView(discord.ui.View):
    ...
    def __init__(self, user_id, channel_id, user, bot, quiz_id, timer, user_name=None):
        ...
        self.channel_id = channel_id  # Channel to report results to
        self.user = user  # User object to DM
        self.bot = bot  # Bot instance for accessing channels
        self.score = 0
        self.index = 0
        self.message = None  # Will store the DM message
        self.questions = []
    ...
    async def initialize(self, quiz_id):
        ...
        logger.debug(f"Initializing DM quiz {quiz_id} for user {self.user_id}")
    ...
    async def end_quiz(self):
        """Ends the quiz and displays results to user DM and reports back to channel"""
        ...
        # Send DM results
        await self.message.edit(content="Quiz finished!", embed=dm_embed, view=None)
        ...
        try:
            ...
            if channel:
                ...
                logger.error(f"Channel {self.channel_id} not found for reporting results")
    ...
    async def show_question(self):
        ...
        if not self.questions:
            ...
            await self.user.send("No questions found for this quiz. Please check the database.")
            return
        ...
        for key in options.keys():
            button = DMQuizButton(key, question_data, self)
...
class DMQuizButton(discord.ui.Button):
    """Button for DM quiz answers"""
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        async with self.quiz_view.lock:
            max_score = self.question_data[5]  # Maximum possible score
